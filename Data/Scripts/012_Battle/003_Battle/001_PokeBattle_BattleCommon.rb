module PokeBattle_BattleCommon
  ...
  def pbStorePokemon(pkmn)
    ...
    if !pkmn.shadowPokemon?
      if Nuzlocke.on? || pbDisplayConfirm(_INTL("Would you like to give a nickname to {1}?",pkmn.name))
  ...
  def pbRecordAndStoreCaughtPokemon
    @caughtPokemon.each do |pkmn|
      ...
      if !pbPlayer.hasOwned?(pkmn.species)
        ...
        if $Trainer.pokedex
          pbDisplayPaused(_INTL("{1}'s data was added to the Aisho Journal.",pkmn.name))
  ...
  def pbThrowPokeBall(idxBattler,ball,rareness=nil,showPlayer=false)
    ...
    when 4
      ...
      if(pkmn.shadowPokemon?)
        pbMessage(_INTL("Gotcha! \\c[9]Shadow {1}\\c[0] was recovered from the \\c[9]Akui Clan\\c[0]!",pkmn.name))
      else
