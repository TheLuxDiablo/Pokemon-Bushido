class PokeBattle_AI
  ...
  def pbRegisterMoveTrainer(idxMove,choices)
    ...
    if PBTargets.multipleTargets?(targetType)
      ...
      @battle.eachBattler do |b|
        ...
        score = pbGetMoveScore(move,@user,b)
        totalScore += ((@user.opposes?(b)) ? score : -score)
      end
      choices.push([idxMove,totalScore,-1]) if totalScore>0
    elsif PBTargets.noTargets?(targetType)
      # If move has no targets, affects the user, a side or the whole field
      score = pbGetMoveScore(move,@user,@user)
  ...
  #=============================================================================
  def pbGetMoveScore(move,user,target)
    score = 100
    ...
    if move.damagingMove?
      score = pbGetMoveScoreDamage(score,move,target)
    else   # Status moves
  ...
  #=============================================================================
  def pbGetMoveScoreDamage(score,move,target)
