module FollowingPkmn
  ...
  def self.item(item, quantity = 1, message = nil)
    ...
    if $PokemonBag.pbStoreItem(item, quantity)   # If item can be picked up
      ...
      if isConst?(item,PBItems,:LEFTOVERS)
        ...
        pbMessage(_INTL("\\me[{1}]{4} found \\c[1]{2} {3}\\c[0]!\\wtnp[30]", meName, itemname, PBMoves.getName(move), pokename))
      elsif quantity>1
    ...
    if item == :LEFTOVERS
      ...
      pbMessage(_INTL("{1} found \\c[1]{2} {3}\\c[0]!\\wtnp[30]", pokename, itemname, PBMoves.getName(move)))
    elsif quantity>1
  ...
  def self.increase_time
    ...
    $Trainer.firstAblePokemon.changeHappiness("levelup") if ($PokemonGlobal.time_taken % friendship_time) == 0
