module FollowingPkmn
  ...
  def self.start_following(event_id = nil, anim = true)
    ...
    return false if !$Trainer.firstAblePokemon || !$game_map.events[event_id]
  ...
  def self.toggle(forced = nil, anim = nil)
    ...
    if !forced.nil?
      forced = true if forced == "on"
      forced = false if forced == "off"
...
def followingMoveRoute(*args)
  FollowingPkmn.move_route(*args)
end

def pbToggleFollowingPokemon(*args)
  args[1] = false if $game_switches[1]
  FollowingPkmn.toggle(*args)
end

def pbPokemonFollow(*args)
  FollowingPkmn.start_following(*args)
end
