...
def pbReceiveItem(item, quantity = 1)
  ...
  itemname = (quantity>1) ? PBItems.getNamePlural(item) : PBItems.getName(item)
  pocket = pbGetPocket(item)
  meName = $PokemonBag.pbCanStore?(item,quantity) ? ((pbIsKeyItem?(item)) ? "PLA 014 Caught a Pokemon!" : "PLA 036 Obtained a Key Item!") : ""
  ...
  if isConst?(item,PBItems,:LEFTOVERS)
    ...
    pbMessage(_INTL("You were given the \\c[1]{1}\\c[0]!\\wtnp[30]",itemname))
  elsif isConst?(item,PBItems,:LUNARWING)
    pbMessage(_INTL("You found the mythical \\c[1]{1}\\c[0]!\\wtnp[30]",itemname))
  ...
  elsif isConst?(item,PBItems,:CAMLETTER) || isConst?(item,PBItems,:LUXLETTER) || isConst?(item,PBItems,:TRISTANLETTER) ||
    isConst?(item,PBItems,:HAUNTEDLETTER) || isConst?(item,PBItems,:GOLILETTER) || isConst?(item,PBItems,:ENLSLETTER) || isConst?(item,PBItems,:VOLTSLETTER)
