...
class ModularTitleScreen
  ...
  # entries in the MTS_MODIFIERS array
  def initialize
    # defines viewport
    ...
    @sprites["start"].bitmap = pbBitmap("Graphics/Titles/ModularTS/start")
    @sprites["start"].center!
    @sprites["start"].x = @viewport.rect.width/2
    @sprites["start"].x = MTS_START_POS[0] if MTS_START_POS[0].is_a?(Numeric)
    @sprites["start"].y = @viewport.rect.height*0.85
    @sprites["start"].y = MTS_START_POS[1] if MTS_START_POS[1].is_a?(Numeric)
    @sprites["start"].z = 999
